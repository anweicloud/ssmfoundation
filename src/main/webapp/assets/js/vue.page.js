var Pagenation = function(el) {
	
	el = document.getElementById(el);
	el.innerHTML += ('<pagination :total="total" :current.sync="current"></pagination>');

    var template_pagination = '<nav>\
	        <ul class="pagination">\
    	...
    	methods: {
    		setCurrent: function(idx) {
    			if( this.current != idx && idx > 0 && idx < this.page + 1) {
    				this.current = idx;
    				eventHub.$emit('pagechange',this.current);                	
    			}                
    		}
    	}
    });
